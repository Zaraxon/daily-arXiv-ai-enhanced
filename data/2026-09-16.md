...
TL;DR: 论文指出现有发生类型系统因限制类型中可出现的项而破坏替换性质，导致Typed Racket核心形式化及其扩展工作存在缺陷；作者修复该问题，并在Lean中以步数索引逻辑关系完成语义类型健全性证明。


<details>
  <summary>Details</summary>
Motivation: 精化类型、发生类型等技术通过受限依赖关系为多种语言引入依赖类型能力，但对类型内项的限制使任意项替换变量不再成立。这会显著增加元理论设计复杂度，并可能使类型健全性证明及实现产生错误，因此需要重新审视Typed Racket发生类型演算的理论基础。

Method: 作者聚焦Tobin-Hochstadt与Felleisen于2010年提出的Typed Racket底层发生类型演算，分析类型中替换失败所引发的形式化漏洞，检查这些漏洞在后续论文和Typed Racket实现中的传播情况。随后，作者修改并修复核心演算，采用步数索引逻辑关系建立语义类型健全性证明，并将该证明在Lean中形式化验证。

Result: 研究发现原始形式化及其句法类型健全性定理包含多个缺陷，相关问题也被后续若干研究继承，并在Typed Racket实现中表现为实际的健全性错误。修订后的核心演算能够避免这些问题；Lean形式化结果表明，步数索引逻辑关系方法可处理发生类型中的复杂机制。

Conclusion: 限制类型内项虽然能扩展类型系统表达力，却会破坏传统替换引理，不能直接沿用常规句法健全性证明。对于Typed Racket的发生类型，基于语义的步数索引逻辑关系提供了更可靠且可扩展的验证路径，并能通过机器辅助证明降低元理论错误风险。
...
TL;DR: 论文提出Exo-GPU：一种接近CUDA底层能力的命令式GPU语言。其核心将并行与同步视为顺序程序上的注解，并验证并行执行与顺序语义等价；在H100上的GEMM实现达到大规模问题理论峰值80%以上，部分情况下超过CUBLAS。


<details>
  <summary>Details</summary>
Motivation: 现代GPU性能依赖SIMT并行及计算与数据搬运之间的软件显式并发。现有方案存在两难：CUDA内联汇编和内建函数提供细粒度控制但缺少安全保证；Triton等高级模型更易分析，却可能将异步细节隐藏在编译器后端，限制专家对关键流水与同步细节的调优。

Method: Exo-GPU采用命令式、低抽象语言设计，将线程、warp、warpgroup、block、cluster等层级并行性，以及异步张量核和内存拷贝操作相关同步，表达为顺序代码上的注解，而非改变语义的控制流原语。编译器据此验证顺序—并行等价性，确保并行执行在功能上等同于顺序解释。作者使用wgmma、TMA和split-k等H100特性编写GEMM内核。

Result: 在NVIDIA H100上，Exo-GPU编写的GEMM内核面对大规模矩阵问题实现了超过理论峰值80%的性能；若干测试场景中，其性能超过厂商提供的CUBLAS库。

Conclusion: Exo-GPU表明，低层GPU性能控制与可验证语义安全可以结合：通过以顺序语义为基础、将并行和同步显式注解化，开发者能够调优异步执行细节，同时由编译器保证功能正确性。该方法尤其适合需要利用新一代GPU专用指令和多级存储流水的高性能内核开发。
...
### [5] [Universal Defenses for Tool-Integrated LLM Agents Against Adversarial Attacks](https://arxiv.org/abs/2609.16098)
*Xiaoyan Li,Yunli Wang*

Main category: cs.CR

TL;DR: 论文提出统一的工具集成LLM智能体防御框架，针对直接/间接提示注入、记忆投毒和后门四类对抗攻击，结合工具层与提示层防御以降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 工具增强型LLM智能体依赖开放提示和外部工具执行多步任务，因此易被恶意指令、异常工具或被污染记忆操控。现有防御往往仅覆盖单一攻击面，缺乏可迁移、模块化的统一方案。

Method: 工具层包括：1）Attacker Tool Filtering，利用Isolation Forest等异常检测识别并移除可疑工具；2）Normal Tool Recalling，在规划前恢复智能体原始、可信的工具集合。提示层包括链式思维提示、自我反思及任务改写，以增强安全推理并削弱恶意指令影响。

Result: 在Gemma2-9B、Qwen2-7B、LLaMA3-8B、LLaMA3.1-8B以及GPT-3.5、GPT-4、GPT-5上，所提方法显著降低四类攻击的攻击成功率；多个实验设置达到0%攻击成功率，同时保持或提升原始任务成功率。

Conclusion: 该工作表明，简单的工具白名单/异常过滤与提示增强相结合，可形成有效的多层防线。其主要价值在于跨攻击类型和跨模型的通用性，但摘要未说明攻击强度、误报率、工具动态变化场景及真实生产环境下的计算开销，仍需进一步验证。
...
### [6] [Permutation-Based Stegomalware in Large Language Models: Threats and Countermeasures](https://arxiv.org/abs/2609.16193)
...
### [7] [Feasibility of Homomorphic Inference for a Genomic Foundation Model](https://arxiv.org/abs/2609.16211)
...
### [8] [Analyzing Multi-Factor Authentication Through Cryptographic Security Properties](https://arxiv.org/abs/2609.16214)
...
### [9] [RuleAutoPilot: Synthesizing Deployable Suricata Rules from Network Traffic](https://arxiv.org/abs/2609.16231)
...
### [10] [Exploiting and Securing Docker containers and Kubernetes pods from a MitM attack](https://arxiv.org/abs/2609.16253)
...
### [11] [Understanding the Usability of Cryptographic Verification Tools](https://arxiv.org/abs/2609.16323)
...
### [12] [Illusion of Depth: Revealing Hidden Stereo Vision Vulnerabilities in Depth Estimation](https://arxiv.org/abs/2609.16336)
...
### [13] [gr-PHYSEC: Real-time Channel-based Key Generation for Physical Layer Secure Wireless Communications](https://arxiv.org/abs/2609.16375)
...
### [14] [Implementing a White-Box Undetectable Backdoor for Random Fourier Features](https://arxiv.org/abs/2609.16403)
...
### [15] [No Bit Left Behind: Using Brute-Force Lifting to Achieve Fully Static Binary Recompilation](https://arxiv.org/abs/2609.16423)
...
### [16] [Evaluating the NIST Bugs Framework Against CWE as a Successor for Automated Vulnerability Classification](https://arxiv.org/abs/2609.16433)
...
### [17] [Not All Relations Are Equal: Relation-Balanced and Calibrated Graph Learning for Provenance-Based Intrusion Detection](https://arxiv.org/abs/2609.16462)
...
### [18] [A Cyber Range Evaluation of Autonomous Network Incident Response Agents](https://arxiv.org/abs/2609.16541)
...
### [19] [GPUThor: Amplifying Rowhammer Attacks via Non-Uniform Patterns to Exploit ECC-Protected GPUs](https://arxiv.org/abs/2609.16546)
...
### [20] [The MAL Simulator: Cyber Operations Simulation based on Attack & Defense Graphs](https://arxiv.org/abs/2609.16563)
...
### [21] [From Hypervisor to Container: Cloud Security Vulnerabilities, Defense Mechanisms, and Open Challenges](https://arxiv.org/abs/2609.16675)
...
### [23] [Toward Secure AI-Powered Penetration Testing Agents: Security Threats, Guardrails, and Architectural Perspectives](https://arxiv.org/abs/2609.16694)
...
### [24] [When Agents See Differently: Exposing UI Desynchronization Threats in Mobile Agents](https://arxiv.org/abs/2609.16732)
...
### [25] [InceptionRAG: Stealthy Poisoning Attack Against Retrieval-Augmented Generation](https://arxiv.org/abs/2609.16818)
...
### [26] [ROSETTA: Efficient and Accurate Privacy-Preserving LLM Decoding via Hybrid CKKS/TFHE Evaluation](https://arxiv.org/abs/2609.16915)
...
### [27] [Cybersecurity in Power Grids: Standards and Research Challenges](https://arxiv.org/abs/2609.16928)
...
### [28] [Observational Indistinguishability and Integrity Blind Regions in Hybrid Quantum-Classical Workflows](https://arxiv.org/abs/2609.17150)
...
### [29] [Plug 'n' Pray: Agentic LLM-based Detection of Potential Log File Exposures in Third-Party Content Management System Plugins](https://arxiv.org/abs/2609.17164)
*Sebastian Neef*
...
### [30] [Cross-Domain Inference for Human Localization: Applying Wi-Fi RSSI Data to CSI-Trained Models](https://arxiv.org/abs/2609.17204)
...
### [31] [SEMA-GUARD: Semantic and Graph-Based Vulnerability Detection in Assembly Code](https://arxiv.org/abs/2609.17254)
...
### [32] [GAUGE: A Formal Framework for Measuring Cryptographic Security under Heterogeneous Adversary Cost Models](https://arxiv.org/abs/2609.17281)
...
### [33] [Can We Stop The Ads? Taxonomy and Characterization of Smartphone Splash Ads and Existing Countermeasures](https://arxiv.org/abs/2609.17316)
...
### [34] [RobResilience: Implementing and Evaluating a Resilience Framework for Cyber-Physical Embodied Systems](https://arxiv.org/abs/2609.17349)
...
### [35] [Closing the Loop: Bidirectional Fully Encrypted Protocols](https://arxiv.org/abs/2609.17397)
...
### [36] [SCHERI: Provably Secure Speculation Under the Constant-Time Policy for CHERI (Extended Version)](https://arxiv.org/abs/2609.17399)
...
### [37] [You Shall Not Pass into Ring-0! A User Privacy-Friendly Anti-Cheat Architecture for Personal Computers](https://arxiv.org/abs/2609.17525)
...
### [38] [Teaching Vampire New Tricks: An Experimental Study of Neural Clause Selection](https://arxiv.org/abs/2609.16228)
*Karel Chvalovský,Martin Suda,Josef Urban*

Main category: cs.LO

TL;DR: 论文评估了Vampire定理证明器中的神经子句选择引导在多个由交互式定理证明器衍生的基准集上的效果，并考察其与不同证明策略的交互。


<details>
  <summary>Details</summary>
Motivation: 此前该神经引导方法已在TPTP基准上显著提升Vampire默认策略的成功率，但其在其他领域、跨数据集泛化能力，以及与多种证明策略组合时的实际收益尚不明确。

Method: 作者在多个ITP衍生基准集上进行实验，对比默认策略与神经引导策略在单一领域内、跨基准迁移、统一多数据集训练及多策略组合等设置下的性能表现。

Result: 神经引导在每个独立基准领域内均能稳定提升性能；但将在某一基准训练的模型直接用于其他基准时，性能甚至低于未使用引导的默认策略。将所有数据集联合训练为单一模型可缓解这一问题，尽管训练成本更高，但能使Vampire在各数据集上的总体表现接近最佳水平。多策略组合的结果则不够明确，神经引导仍有价值，但额外收益呈递减趋势。

Conclusion: 神经子句选择对领域内自动定理证明具有可靠效果，但跨领域泛化不足。多数据集联合训练是提升泛化性的有效方案；然而，在已有策略组合较强时，神经引导带来的边际改进有限，后续工作应关注更高效的泛化训练与策略协同机制。
...
### [39] [Vibe-Coded and Tuned: A State-of-the-Art SMT Solver for QF-LRA](https://arxiv.org/abs/2609.16706)
*Mikoláš Janota,Jan Jakubův*

Main category: cs.LO

TL;DR: 该摘要介绍了名为 primo 的 QF-LRA SMT 求解器，声称通过文献调研、性能剖析与参数调优获得领先性能，并超过 SMT-COMP 2026 QF-LRA 赛道冠军。


<details>
  <summary>Details</summary>
Motivation: 目标是在量词自由线性实数算术问题上提升 SMT 求解性能，同时探索以“vibe-coded”方式快速构建自动推理工具的可行性。

Method: 作者采用系统性文献调查确定设计方向，通过反复 profiling 定位性能瓶颈，并进行参数调优；但摘要未说明核心算法、实现架构、基准集划分、调优预算、硬件环境及统计检验方法。

Result: 摘要声称 primo 在 QF-LRA 上达到最先进水平，并优于 SMT-COMP 2026 对应赛道获胜工具。该结论目前仅为摘要中的性能声明，缺少可复现实验数据、具体指标和与基线工具的公平比较设置。

Conclusion: 论文的潜在贡献是展示工程化调优可显著提高 SMT 求解器性能，并提出自动代码生成式开发可能促进自动推理工具迭代。然而，关于领先性能及其对开发范式的广泛推论，需要由完整算法描述、公开代码与配置、独立测试集及严格可复现实验进一步支撑。
...
### [40] [Universal Properties of Petri Net Unfoldings](https://arxiv.org/abs/2609.17324)
...
### [41] [Refining Timing Uncertainty from Logical Time Specification to Operation](https://arxiv.org/abs/2609.17388)
...
### [42] [Predictable Modelling and Analysis of Software-defined Vehicle Implementations](https://arxiv.org/abs/2609.17392)
*Pavlo Tokariev,Yosri Ayari,Julien Deantoni*

Main category: cs.LO

TL;DR: 论文验证了面向软件定义车辆的概率时序架构模型能否真实反映基于Kuksa中间件的实现行为，并表明该模型在不同中间件负载下兼具保守时序保证与较好的预测代表性。


<details>
  <summary>Details</summary>
Motivation: 软件定义车辆中的中间件通信和硬件抽象会引入时延不确定性，影响端到端时序保证。已有设计阶段的概率模型虽可用于早期分析，但其与实际平台实现之间的一致性和代表性尚未得到充分验证。

Method: 作者构建实验框架，将概率化设计阶段时序分析与受监控的Kuksa实现结合。对仿真轨迹和实现执行轨迹采用相同的反应时间分析，并通过比较方法区分两类指标：模型是否保守覆盖实际时序结果，以及模型预测的时序分布与实际分布之间的拟合程度。

Result: 实验显示，在不同中间件负载条件下，所提出的架构抽象仍能代表实际实现的时序行为；同时，分析结果保持了保守性，即预测的时序保证能够覆盖观测到的实现行为。

Conclusion: 概率时序架构模型可作为软件定义车辆早期时序验证的有效抽象，并可支持从设计阶段模型到实际平台监测的增量式时序验证流程。
...
### [43] [Optimal Pruning for Neural Architectures using Fisher Information Distances](https://arxiv.org/abs/2609.16129)
*David S. Berman,Yen-Yu Fu,Edward Hirst,Thelma Chiwete Obirai*

Main category: cs.AI

TL;DR: 论文提出基于模型空间中Fisher信息度量所定义测地距离的参数剪枝框架，将剪枝视为把参数置零并投影到对应参数为零的超曲面；该方法在多个网络、数据集和剪枝率下优于幅值剪枝及仅使用局部Fisher信息的剪枝。


<details>
  <summary>Details</summary>
Motivation: 传统幅值剪枝以参数绝对值衡量重要性，无法直接反映置零后模型输出分布及性能的真实变化；局部Fisher信息方法虽考虑参数敏感性，但仍是局部近似。论文旨在用具有明确几何含义的距离度量，建立更可靠且可验证的参数重要性标准。

Method: 作者将模型参数空间赋予Fisher信息度量，把单个参数剪枝定义为从当前模型到“该参数等于零”超曲面的位移。参数的重要性由到该超曲面的最小测地距离决定。随后对精确测地距离构造逐级近似，由此形成剪枝最优性层级：最低层对应传统幅值剪枝，更高层结合Fisher度量及更准确的几何修正，并在计算代价与剪枝质量之间提供不同折中。

Result: 在全连接网络和视觉Transformer上，针对MNIST与CIFAR-10，覆盖0%至100%剪枝比例并使用五个随机种子评估。结果显示，所提出的几何剪枝方法在所有报告的架构—数据集组合中，均同时优于参数幅值剪枝和仅依赖局部Fisher信息的剪枝，评价指标包括准确率与Matthews相关系数。部分中间层级的测地距离近似还能以较低计算成本取得接近最优的性能。

Conclusion: 该工作将参数剪枝统一解释为Fisher-Rao模型几何下的最短距离问题，为幅值剪枝及其改进形式给出层级化的理论依据。其核心贡献不仅是获得更优的实证剪枝效果，也是在性能、计算效率和数学可解释性之间建立了系统化的剪枝设计框架。
...
### [44] [Safe Error Correction for Language Models: Frozen-Base Adjustment with Capability Preservation](https://arxiv.org/abs/2609.16145)
*Gautam Kishore*

Main category: cs.AI

TL;DR: 论文提出CRN v2：在完全冻结的4.65B文本模型输出logit层上附加约3400万参数的轻量纠错模块。其核心结论是，冻结底座、logit级纠错与KL保持约束可在修正部分错误的同时维持已测基础能力；但在60题小规模领域测试上，最佳纠错率为53.3%，仍显著低于会造成能力退化的LoRA基线。


<details>
  <summary>Details</summary>
Motivation: 研究关注模型纠错中的“纠错—能力保持”权衡：直接微调虽可能提升特定纠错任务表现，却可能损害模型原有通用能力。作者希望验证能否只训练一个小型外挂模块，在不改动底座参数的条件下修复输出错误，并尽量保持原始行为与能力。

Method: CRN v2置于冻结模型的logit输出层，仅训练纠错模块；训练分为监督微调和无参考DPO两阶段，使用83,400个错误纠正样本对。方法通过KL项约束纠错后分布不要过度偏离底座输出，KL系数设为0.1。实验在60题CEHRI领域考试上评估错误修正率，并以MMLU、BoolQ各200题及8题car-wash任务检查能力保持。对比包括同参数预算LoRA、不同深度隐藏状态注入、不同logit校正深度、更长训练，以及不同KL系数等消融配置。

Result: 主结果显示，rank-128的logit纠错模块可修正53.3%的底座错误，措辞改写版本为43.3%，且在所测能力基准上未观察到退化。匹配CRN v1参数预算的LoRA虽达到83.3%纠错率，但在能力测试中出现30%至75%的性能损失。KL保持项至关重要：系数从0.1降到0.01后，纠错率降至35.0%。早层隐藏状态注入方案以更少参数取得50.0%/55.8%的结果，但作者认为总体未优于logit方案；更浅层注入、跨深度logit校正和延长训练均未超过53.3%。

Conclusion: 该工作较有力地支持了一个工程设计原则：对于希望保留既有能力的定向纠错，可采用冻结底座、输出logit级外挂校正和KL锚定，而非直接更新底座。其主要局限是纠错评估集仅60题、能力保持测试规模有限，且“无退化”只能解释为在已测任务和样本上未发现明显退化，不能证明全面能力不受影响。此外，最佳53.3%的纠错率表明该方案尚不能可靠覆盖多数错误；与LoRA相比，它体现的是能力保持优势，而非绝对纠错效果优势。
...
### [45] [GPEvac: GNN-Based PPO for Adaptive Evacuation Routing During Shooting Events](https://arxiv.org/abs/2609.16163)
...
### [46] [Position: AI Is Not Ready for Strategic Conflicts](https://arxiv.org/abs/2609.16189)
...
### [47] [Calibrate, Then Route: A Measured Study of Learned Request Routing for Disaggregated LLM Serving](https://arxiv.org/abs/2609.16206)
...
### [48] [Artificial intelligence and biosecurity: capabilities, threat pathways, and defense-in-depth governance](https://arxiv.org/abs/2609.16213)
...
### [49] [Where Should the KV Cache Live? Placement Policies Across GPU, CPU, and SSD for Long-Lived Sessions](https://arxiv.org/abs/2609.16215)
*Srikanta Datta Tumkur,Jay Iyer,Mehar Simhadri,Sai Pavan Kumar,Sai Kapil Kumar,Ramesh Nampelly*
...
### [50] [Toward Governance-Aware Autonomous GIS: A Narrative Review of Ethical and Privacy Risks in LLM-Enabled GeoAI](https://arxiv.org/abs/2609.16232)
...
### [51] [Metacognitive Steering: Learning the Structure of Scientific Judgment](https://arxiv.org/abs/2609.16245)
...
### [52] [The Pain Axis: LLMs Represent Self-Directed Harm and Act to Relieve It](https://arxiv.org/abs/2609.16247)
...
### [53] [CADWorld: Computer-Use Benchmark for Long-Horizon Computer-Aided Design](https://arxiv.org/abs/2609.16251)
...
### [54] [The AI-Enabled Scientific Frontier](https://arxiv.org/abs/2609.16258)
...
### [55] [Closing the Loop: Branch-and-Bound for Scalable Verification of Nonlinear Neural Feedback Systems](https://arxiv.org/abs/2609.16298)
...
### [56] [CLEAR: Cross-Source Evidence Adjudication for Large Language Models in Medicine](https://arxiv.org/abs/2609.16301)
...
### [57] [BLINDSPOT: A Benchmark for Safety and Refusal Calibration in Long-Horizon Tool-Using Agents](https://arxiv.org/abs/2609.16305)
...
### [58] [Cross-Anatomy Transfer Versus Sparse Interpolation in Digital-Twin-Oriented Aortic Fluid-Structure Interaction Surrogates](https://arxiv.org/abs/2609.16322)
...
### [59] [Breaking the 1.58-bit Barrier for Ternary LLMs](https://arxiv.org/abs/2609.16338)
...
### [60] [Fine-Tuning Fixes Mode Collapse and Over-Dispersion in LLMs](https://arxiv.org/abs/2609.16454)
...
### [61] [Skill-based Agentic Evaluation for Real-time Data Science Tasks](https://arxiv.org/abs/2609.16487)
...
### [62] [From Manual Construction to AI-Driven Scenario Emergence: Rethinking Catastrophe Risk Modeling](https://arxiv.org/abs/2609.16493)
*Hang Gao*
...
### [63] [AquiLLM: Evaluating Faithfulness in Open-Weight RAG-LLM Systems for Scientific Research](https://arxiv.org/abs/2609.16519)
...
### [64] [QueryFormer: Winning Solution for KDD Cup 2026 Tencent UniRec Challenge](https://arxiv.org/abs/2609.16548)
...
### [65] [Query-Aware Source-Risk Triage for Retrieval-Augmented Generation](https://arxiv.org/abs/2609.16564)
...
### [66] [Do LLMs Have Values? A Quantitative Analysis and Alignment Framework for Values in Large Language Models](https://arxiv.org/abs/2609.16589)
...
### [67] [A Framework for Generating Valid Context-Specific Benchmarks through Expert Guidance](https://arxiv.org/abs/2609.16592)
...
### [68] [EchoPath: Execution-Level Replayable Memory for GUI Agents](https://arxiv.org/abs/2609.16635)
...
### [70] [ANIMASK: What the Model Contributes to Role Play in Simulated Story Worlds](https://arxiv.org/abs/2609.16667)
...
### [71] [AI for Games in the Foundation Model Era](https://arxiv.org/abs/2609.16679)
...
### [72] [little m: An AI Agent for Industrial Process Optimization](https://arxiv.org/abs/2609.16680)
...
### [75] [Beyond Episodic AI: Cognitive Field Networks for Biologically Inspired Persistent Cognition](https://arxiv.org/abs/2609.16752)
*Byung Gyu Chae*
...
### [76] [Turn-level Multiscale Density Ratio Estimation for LLM Agents](https://arxiv.org/abs/2609.16760)
...
### [77] [Coverage-Aware Virtual IMU Augmentation for Low-Resource Human Activity Recognition](https://arxiv.org/abs/2609.16768)
...
### [78] [Integrating the Analytic Hierarchy Process with Large Language Models for Transparent Multi-Criteria Decision-Making](https://arxiv.org/abs/2609.16779)
...
### [79] [Layers, Sinks, and Scaling: Adaptive Evidence Selection for Multimodal Large Language Models](https://arxiv.org/abs/2609.16795)
...
### [80] [Can We Do Interpretable NLI with Graphs Based on Atomic Propositions?](https://arxiv.org/abs/2609.16814)
...
### [81] [Execution Flexibility in Automated Planning: A Comparative Evaluation of Deordering and Reordering Strategies](https://arxiv.org/abs/2609.16822)
...
### [82] [CoAdapt: An LLM-based Framework for Adaptive Collaborative Perception in IIoT Robotic Swarms](https://arxiv.org/abs/2609.16852)
...
### [83] [Bridging Learned Visual Perception and Symbolic Belief-Space Planning](https://arxiv.org/abs/2609.16884)
...
### [84] [QART: A Quantum-Classical Hybrid Architecture for Long-Horizon Reasoning -- Exploring a Conditional Path toward Quantum Scaling](https://arxiv.org/abs/2609.16887)
...
### [85] [AntennaFlow: A Generative Flow Model for Offset Correction in Phaseless Antenna Testing](https://arxiv.org/abs/2609.16948)
...
### [86] [Affect-Prototype Guided Fusion for Open-Vocabulary Incomplete Multi-modal Emotion Recognition](https://arxiv.org/abs/2609.16962)
...
### [87] [FlexEE: Self-Speculative and KV-Compatible Early Exiting for Offloading-Aware LLM Inference](https://arxiv.org/abs/2609.17008)
...
### [88] [ThinkFlow: Self-Evolving Probabilistic Latent Memory for Lifelong Conversational Agents](https://arxiv.org/abs/2609.17010)
...
### [89] [ORDER: Task-Conditioned Routing for Retrieval-Augmented Generation](https://arxiv.org/abs/2609.17012)
...
### [90] [SKIP: a Self-knowledge-guided Step-wise Preference Learning Framework for Concise Reasoning](https://arxiv.org/abs/2609.17019)
...
### [91] [Sparse MLLM Anchors, Dense Adaptation: Breaking the Self-Referential Loop in Wild Test-Time Adaptation](https://arxiv.org/abs/2609.17040)
...
### [92] [Neuro-Symbolic Hierarchical Intention Anticipation in Human Behavior](https://arxiv.org/abs/2609.17064)
...
### [93] [Sample-Conditioned Representation Selection for Audio Few-Shot Learning](https://arxiv.org/abs/2609.17076)
...
### [94] [Interactive Memory Learning for Long-Term Conversations](https://arxiv.org/abs/2609.17088)
...
### [95] [Scaling-Score Conformal Prediction for Multi-Target Regression](https://arxiv.org/abs/2609.17091)
...
### [96] [Semi-Supervised Learning-Based Genetic Biomarkers Dataset for Multiple-Stage Hepatocellular Carcinoma Prediction](https://arxiv.org/abs/2609.17100)
...
### [97] [Symbolic Separation: Grounding Deep Agents in Knowledge Graphs for Trustworthy Operational Data Analytics](https://arxiv.org/abs/2609.17107)
...
### [98] [Shared-Prefix KV Reuse Across Standard LoRA Adapters: Quality and Serving Tradeoffs](https://arxiv.org/abs/2609.17109)
*Dushyant Rajput*
...
### [99] [FirmCORe: A Benchmark for Structured Reasoning about Inter-Firm Collaboration Opportunities](https://arxiv.org/abs/2609.17128)
...
### [100] [MOCC-R1: Reinforcing Reasoning-Response Consistency for Multimodal Counselor Response Generation](https://arxiv.org/abs/2609.17180)
...
### [101] [End-to-End Latency-Minimizing and Load-Balanced Request Scheduling for Edge LLM Inference in Agentic AI Services](https://arxiv.org/abs/2609.17193)
...
### [102] [Extracting ontology-compliant knowledge from scientific text describing irradiated materials using large language models](https://arxiv.org/abs/2609.17291)
...
### [103] [Intrinsic Motivation in Reinforcement Learning: A Research Agenda for Adaptive Self-Organisation](https://arxiv.org/abs/2609.17325)
*Anatoly Belikov*
...
### [104] [From Transient Prompts to Persistent Control: Scientific Poster Generation via Recursive Semantic-Geometric Contracts](https://arxiv.org/abs/2609.17326)
...
### [105] [Self-Emergence Agent Architecture:Behavior-Inertia HMM, Reflexive Metacognition,and Social-Contrastive Self-Modeling](https://arxiv.org/abs/2609.17331)
*Xiaoyang Liu*
...
### [106] [FlashVector: Agent for Hierarchical Model Serving Stack Optimization](https://arxiv.org/abs/2609.17391)
...
### [107] [Never Stop Thinking: Continuous-Time Language Agents](https://arxiv.org/abs/2609.17416)
*Bojie Li,Noah Shi*
...
### [108] [World Model Science: Self-Organized Criticality, Weak Chaos, and Metastable Belief Dynamics in Long-Horizon LLM Agents](https://arxiv.org/abs/2609.17419)
...
### [109] [Transformer-Based Token Fusion and Dynamic Graph Planning for Audio-Visual Navigation](https://arxiv.org/abs/2609.17421)
...
### [110] [Talking Head Synthesis with Facial Landmark Guidance via 3D Gaussian Splatting](https://arxiv.org/abs/2609.17422)
...
### [111] [JustFit: 200K-Token LLM Serving on a 24 GiB Laptop with Just-in-Time State Management](https://arxiv.org/abs/2609.17475)
*Yuhua Chen*
...
### [112] [LimiX-2: A Contextual Mechanism Network Towards General Structured-Data Intelligence](https://arxiv.org/abs/2609.17488)
...
### [113] [Verifiable Social Reasoning for LLM Assistants](https://arxiv.org/abs/2609.17496)
...
### [114] [ScienceBuddy: Recursive-in-Recursive Self-Improvement for Interactive Scientific Agents](https://arxiv.org/abs/2609.17523)
...
### [115] [Coaching Qwen3 Coder 30B to Think Like a CodeClash Arena Agent](https://arxiv.org/abs/2609.16096)
*Ivy Ning Zhang*

Main category: cs.SE

TL;DR: 论文研究如何通过强代理蒸馏提升弱/开源代码智能体在长程、多轮交互式编程任务中的表现，并以 Qwen3-Coder-30B 为案例提出 ReAct SFT 与轨迹质量加权 SFT。


<details>
  <summary>Details</summary>
Motivation: 弱代码智能体在代码竞技场类任务中难以稳定理解意图、利用历史交互结果诊断失败并选择后续编辑。其常出现语法错误、交互协议违规及跨轮策略适应不足。传统离线监督微调无法直接判断生成动作是否有效或有益，因此需要更适合交互轨迹学习的蒸馏方法。

Method: 作者基于 CodeClash 基准中的强智能体轨迹进行知识蒸馏。首先，将教师轨迹改写为显式的“[obs][thought][act]”链式结构，使模型学习观察环境反馈、进行推理并采取代码操作的过程，称为 ReAct SFT。其次，采用轨迹质量加权 SFT，对更能体现编辑后检查与有效决策的训练样本赋予更高权重，从而强化动作有效性与后验验证行为。

Result: 分析表明，原始 Qwen3-Coder-30B 并未针对竞技场式多轮交互充分优化，存在较多格式、语法和协议错误，且策略调整能力较弱。ReAct SFT 显著改善了其多轮战略行为；结合所提出的训练改进后，微调模型在锦标赛评测中超过了原始 Qwen3 Coder Plus。

Conclusion: 对于长程交互式代码任务，仅依赖常规指令微调不足以提升弱智能体的可靠决策能力。将强模型轨迹显式结构化为观察—思考—行动过程，并按轨迹质量加权训练，能够有效传递交互策略和编辑后验证能力，进而提升开源代码智能体的竞技场表现。
...
### [116] [Docker Containers vs. Virtual Machines: A Comparative Study of Architecture, Performance, Configuration, and Security](https://arxiv.org/abs/2609.16148)
*Faraz Gurramkonda,Akanksha Malla,Sayma Tamboli,Shayesta Nazneen*

Main category: cs.SE

TL;DR: 该论文基于既有文献比较虚拟机与Docker容器，指出容器通常在启动速度、镜像体积、部署密度和执行效率方面更具优势，而虚拟机提供独立内核、异构操作系统支持及更强隔离边界。二者不存在普适最优解，容器运行于加固虚拟机中的混合架构常是云环境的实用折中。


<details>
  <summary>Details</summary>
Motivation: 现代应用平台需要同时满足工作负载隔离、快速部署、可移植性、资源利用率与安全性。虚拟机和容器分别在硬件虚拟化与操作系统级隔离层面解决这些需求，因此有必要系统分析二者在不同技术指标上的适用边界与权衡关系。

Method: 论文采用文献综述式比较方法，从体系结构、配置与生命周期管理、性能、可扩展性及安全性等维度分析虚拟机和Docker容器。分析同时强调实验结果会受到工作负载类型、存储和网络驱动、资源控制策略及实验设计差异的影响。

Result: 已有研究普遍表明，容器具有更短的启动时间、更小的镜像规模、更高的工作负载密度，并且对许多任务可实现接近原生环境的性能。虚拟机因需要虚拟化硬件并运行独立客户机操作系统而具有较高开销，但能够支持不同客户机操作系统和独立内核，并形成更强的隔离边界。

Conclusion: 虚拟机与容器的核心差异是资源效率和隔离强度之间的设计权衡，而非单一技术优劣之分。对需要较强隔离、异构操作系统或多租户部署的平台，可采用经加固的虚拟机承载容器，以兼顾容器的部署效率与虚拟机的隔离能力。
...
### [117] [Models as Governed Interfaces for AI-Native MBSE: Read-Side Adequacy and Write-Side Admissibility](https://arxiv.org/abs/2609.16252)
...
### [118] [Assurance Envelopes for Autonomous Coding Agents: Minimum-Cost Evidence for Software Change](https://arxiv.org/abs/2609.16302)
*Anjan Goswami*
...
### [119] [AgentGuard: Learning Execution Guardrails from Anomalous Coding-Agent Trajectories](https://arxiv.org/abs/2609.16287)
...
### [120] [FairLint-DL: An IDE-Native Tool for Fairness Debugging of Deep Learning Software](https://arxiv.org/abs/2609.16321)
...
### [121] [Protocol-Preserving Context Trimming for Agentic Workflows: Benefits, Failure Regimes, and Budget Guardrails](https://arxiv.org/abs/2609.16461)
*Harish Gaggar*
...
### [122] [AI Policies: Help or Hindrance? A Software Developer's Perspective](https://arxiv.org/abs/2609.16496)
...
### [123] [ExecuCritic: Calibrated Critic Shaping for Code Generation with Verifiable Rewards](https://arxiv.org/abs/2609.16604)
...
### [124] [An Exploratory Study of Dependabot Cooldown Adoption in Open-Source GitHub Projects](https://arxiv.org/abs/2609.16605)
...
### [125] [Memory-Skill Isomorphism: One Skill Carrier, Two Native Uses](https://arxiv.org/abs/2609.16669)
*Kang Ruiyuan*
...
### [126] [RECTIFY: An Interactive Workbench for Post-Evaluation RAG Diagnosis, Repair, and Verification](https://arxiv.org/abs/2609.16764)
...
### [127] [RepoAtlas: Guiding Coding Agents via Evolving Multimodal Repository Views](https://arxiv.org/abs/2609.16936)
...
### [128] [TasmScan: Continuation-Aware Taint Analysis for TVM Bytecode with Savelist Abstraction](https://arxiv.org/abs/2609.16987)
...
### [129] [Search-Based Metamorphic Testing of Vision-Language Models in Autonomous Underwater Robotic Software](https://arxiv.org/abs/2609.17007)
...
### [130] [GANADI: Uncovering C/C++ OSS Reuse Genealogies via Pivotal Function-Based Clustering to Enhance Supply Chain Security](https://arxiv.org/abs/2609.17018)
...
### [131] [A Set-Theoretic Evaluation Framework for Assessing Asset Administration Shell Instances: Towards Comparability and Suitability](https://arxiv.org/abs/2609.17062)
...
### [132] [Towards an Asset Administration Shell Maturity Model](https://arxiv.org/abs/2609.17084)
...
### [133] [Grounding SWE-Agent Decisions in Architecture-0 Design: Navigating Unknown Unknowns through Physical Mapping](https://arxiv.org/abs/2609.17221)
...
### [134] [A Memorization Floor for LLM Refinement of Decompiled Code](https://arxiv.org/abs/2609.17236)
*Muhammad Asjad*
...
### [135] [An Exemplar of a Digital Twin in Mechanical Engineering: Understanding Model Hybridization](https://arxiv.org/abs/2609.17258)
...
### [136] [After the Party: Governing What a Viral Agent-Skill Ecosystem Left Behind](https://arxiv.org/abs/2609.17274)
...
### [137] [Type-IV Code Clone Detection via Layer-Wise Non-Contrastive Representation Learning](https://arxiv.org/abs/2609.17338)
...
### [138] [Coding Agents Have Converged: Why the SWE-bench Leaderboard Can No Longer Order Its Top Entries, and What to Measure Instead](https://arxiv.org/abs/2609.17394)
